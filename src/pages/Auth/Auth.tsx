  ...
  return (
    <div className='w-full max-w-[1600px] mx-auto min-w-0'>
      <div className='w-full flex flex-col xl:grid xl:grid-cols-2 gap-6 xl:gap-8 2xl:gap-12 min-w-0 xl:items-stretch'>
        ...
        {/* Form */}
        <div className='flex flex-col w-full max-w-[42rem] mx-auto xl:mx-0 xl:max-w-none 2xl:max-w-[42rem] h-full xl:min-h-[58.25rem] rounded-[0.94rem] py-6 sm:py-8 px-4 sm:px-8 lg:px-12 xl:px-10 2xl:px-[107px] border-[0.06rem] border-[#E6E6E6]'>
          <div>
            <h1 className='font-sans text-2xl sm:text-[2.5rem] md:text-[3rem] text-[#222222] font-medium leading-tight'>Welcome to Conflux</h1>
            <p className='font-inter text-base sm:text-lg md:text-[1.25rem] text-[#9D9D9D] mt-2'>Start your experience with signing in or signing up</p>
          </div>
        ...
        <form onSubmit={handleForm} className="flex flex-col flex-1">
          ...
          {isSignIn ? (
            <p className='text-center font-inter text-sm sm:text-base text-[#9D9D9D]'>
              Don't have an account? <Link to="/signup" className='font-medium text-[#0D2D54]'>Register</Link>
            </p>
          ) : (
            <p className='text-center font-inter text-sm sm:text-base text-[#9D9D9D]'>
              Already have an account? <Link to="/signin" className='font-medium text-[#0D2D54]'>Login</Link>
            </p>
          )}
          
          <p className='mt-6 sm:mt-auto pt-4 text-center font-inter font-medium text-xs sm:text-base text-[#9D9D9D] leading-relaxed'>By continuing you are accepting our <span className='text-[#0D2D54]'>Terms & conditions</span></p>
        </form>
      </div>
      </div>
    </div>
